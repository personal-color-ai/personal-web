...
const MainPage = () => {
  ...
  const {data: promptData, refetch, isLoading: promptLoading} = useGetProductPrompt({
    'member-id': memberId,
    prompt: recommended,
    size: 10,
  })
  ...
  const promptProducts = promptData?.pages.flatMap(page => page.result?.list || []) || [];
  ...
  const onClickRecommended = async () => {
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-br from-pink-50 to-violet-50">
      ...
      <div className="p-8">
        {promptData && <div className="mb-6">
          <p className=" text-black text-lg font-semibold mb-5">AI 추천 제품</p>
          <div className="flex gap-3 overflow-x-auto hide-scrollbar">
            {promptProducts.map((product: ProductInfo, idx) => (
              <Card
                key={idx}
                className="overflow-hidden hover:shadow-lg transition-shadow cursor-pointer min-w-[48%]"
                onClick={() => {
    ...
    `}
          ...
          {isPromptLoading && <div>
            <WaveIcon className="text-pink-500"/>
          </div>}
