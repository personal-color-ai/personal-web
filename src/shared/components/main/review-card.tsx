...
const ReviewCard = (props: ReviewCardProps) => {
  ...
  const parsedItems = props.userDescription.split(/·|,/).map(item => item.trim());
  ...
  return (
    <div className="flex gap-3 border border-gray-200 rounded-2xl p-4">
      ...
      <div className="flex-1 overflow-hidden">
        ...
        <div className=" w-full flex gap-2 overflow-hidden my-2 overflow-x-auto scrollbar-none">
          {parsedItems.map((item, i) => (
            <Chips name={item} key={i} />
          ))}
        </div>
        <div>
          <p
